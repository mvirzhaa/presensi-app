...
export async function buildAttendancePdf(event, participants) {
  ...
  // ---- Bagian Notulensi Kegiatan (jika ada) ----
  if (event.notulensi && event.notulensi.trim()) {
    ...
    if (cursorY - requiredHeight < MARGIN + 30) {
      addPage();
    } else {
      cursorY -= 20;
    }

    drawText('Notulensi / Catatan Rapat', MARGIN, cursorY, { font: fontBold, size: 10.5, color: ACCENT });
    cursorY -= 14;
