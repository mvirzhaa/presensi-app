...
export async function ensureSchema(p) {
  ...
  schemaPromise = (async () => {
    try {
      ...
      // 1b. Cek kolom email pada tabel users
      const [userCols] = await p.query(`SHOW COLUMNS FROM users`);
      const userColNames = userCols.map((c) => c.Field);
      if (!userColNames.includes('email')) {
        try {
          await p.query(`ALTER TABLE users ADD COLUMN email VARCHAR(255) NULL`);
        } catch (e) {
          console.warn('Notice adding email column to users:', e.message);
        }
      }
