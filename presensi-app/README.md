...
`require_location`) dan `participants` (relasi 1 event -> banyak peserta).
...
| `NEXT_PUBLIC_BASE_URL` | URL aplikasi, dipakai untuk membentuk link di dalam QR Code. Saat development biarkan `http://localhost:3000`; saat deploy ganti ke domain publik aplikasi |
...
Buka `http://localhost:3000`, lalu klik "Masuk ke Halaman Admin" dan login
...
1. Admin membuka `/admin` → diarahkan ke `/admin/login` jika belum login.
2. Setelah login, admin mengisi form event (termasuk opsi **"Wajibkan
   deteksi lokasi peserta"**), lalu klik "Buat Event & Generate QR".
3. Admin klik event untuk membuka halaman detail (`/admin/event/[id]`) —
...
4. Peserta memindai QR Code → diarahkan ke `/presensi/[id]` yang sudah
...
- ID auto-increment asli (`events.id`) tetap ada di database untuk
  keperluan internal (relasi ke tabel `participants`), tapi **tidak pernah
  dikirim ke browser maupun dipakai di URL/QR Code**.
- Karena acak (bukan urut 1, 2, 3, ...), `public_id` praktis tidak mungkin
  ditebak dengan cara mengganti-ganti angka di URL.
- QR Code yang sudah pernah dicetak sebelum upgrade ini perlu **dicetak
  ulang** dari halaman detail event setelah migrasi database dijalankan
  (lihat catatan migrasi di `schema.sql`), karena link lama berbasis ID
  angka tidak akan dikenali lagi.

## Proteksi Akses ke Pengaturan Event

Seluruh operasi yang mengubah/mengelola event — termasuk toggle wajib-lokasi,
melihat daftar peserta, melihat QR Code, dan export PDF — **hanya bisa
diakses setelah login sebagai admin**:

- Halaman `/admin/*` (termasuk halaman detail/pengaturan tiap event)
  diproteksi oleh `middleware.js`, otomatis mengarahkan ke `/admin/login`
  jika belum login.
- Di sisi server, setiap API yang sensitif (`GET`/`POST /api/events`,
  `PATCH`/`DELETE /api/events/:id`, `GET /api/events/:id/participants`,
  `GET /api/events/:id/qrcode`, `GET /api/events/:id/export`) **juga**
  memeriksa session login — jadi tetap aman walau seseorang mengakses
  URL API-nya secara langsung tanpa lewat halaman admin.
- Hanya dua endpoint yang sengaja tetap publik (tanpa login), karena
  memang dipakai peserta lewat QR Code: `GET /api/events/:id` (info dasar
  untuk ditampilkan di form) dan `POST /api/events/:id/participants`
  (kirim presensi). Keduanya tidak membocorkan data peserta lain maupun
  ID internal database.


## Tabel Daftar Event (Pencarian, Filter, Sort, Pagination, QR)

Bagian "Daftar Event" di halaman admin kini berupa tabel data lengkap
(`components/EventsTable.js`):

- **Pencarian**: satu kotak pencarian bebas yang mencari di kolom nama
  event, lokasi, dan PIC sekaligus.
- **Filter**: dropdown Lokasi dan PIC (otomatis terisi dari data event
  yang ada) untuk mempersempit daftar.
- **Sort per kolom**: klik judul kolom (Nama, Tanggal, Waktu, Lokasi, PIC,
  Peserta) untuk mengurutkan naik/turun. Kolom Peserta diurutkan secara
  numerik (bukan tekstual), jadi 2 < 9 < 10 selalu benar.
- **Pagination**: pilih jumlah baris per halaman (10/20/50), lengkap
  dengan info "Menampilkan X-Y dari Z event" dan tombol
  sebelumnya/berikutnya.
- **Kolom baru "Waktu"**: jam pelaksanaan event (kolom `waktu_event`,
  opsional — tampil "-" jika belum diisi).
- **Thumbnail QR Code** di kolom terakhir setiap baris. Klik thumbnail
  untuk membuka modal QR Code ukuran besar, lengkap dengan:
  - Tombol **"Salin Gambar"** — menyalin gambar QR ke clipboard perangkat
    (lewat Clipboard API browser) sehingga bisa langsung di-paste ke
    aplikasi lain (Word, WhatsApp Desktop, dll). Jika browser tidak
    mendukung penyalinan gambar (beberapa browser lama/Safari versi
    tertentu), akan muncul pesan agar klik-kanan gambar untuk menyalin
    secara manual.
  - Tombol **"Unduh PNG"** sebagai alternatif yang selalu berfungsi di
    semua browser.

Pencarian/filter/sort/pagination ini berjalan di sisi browser (client-side)
dari data yang sudah dimuat, sehingga responsif tanpa perlu memuat ulang
halaman setiap kali mengetik atau berpindah halaman.

## Dua Bahasa (ID/EN)

Kamus terjemahan ada di `lib/i18n.js`, dan dikelola lewat
`components/LanguageProvider.js` (React Context + `localStorage`, tanpa
dependensi tambahan). Saat ini halaman yang sudah diterjemahkan: beranda,
login admin, daftar & buat event, detail event, dan formulir presensi
peserta. **Berkas PDF hasil export saat ini tetap berbahasa Indonesia**
(sebagai dokumen resmi/baku) — beri tahu jika PDF juga perlu mengikuti
pilihan bahasa.
