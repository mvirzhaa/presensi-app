...
export default function PresensiPage() {
  ...
  const [form, setForm] = useState({ nama: '', asal_instansi: '', jabatan: '', catatan: '' });
      ...
      <div className="flex-1 px-4 py-6 sm:py-10">
        <div className="max-w-md mx-auto space-y-5">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 sm:p-6 space-y-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-sm text-slate-600">{t.catatanLabel}</label>
              <textarea
                name="catatan"
                value={form.catatan}
                onChange={handleChange}
                ...
                rows={3}
                className="border border-slate-300 rounded-lg px-3 py-2.5 text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
              />
            </div>
