...
export async function GET(request, { params }) {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, nama, asal_instansi, jabatan, catatan, latitude, longitude, presensi_at
       FROM participants
       WHERE event_id = ?
...
export async function POST(request, { params }) {
  try {
    ...
    const { nama, asal_instansi, jabatan, signature, catatan, latitude, longitude } = body;
    ...
    const [result] = await pool.query(
      `INSERT INTO participants
        (event_id, nama, asal_instansi, jabatan, signature, catatan, latitude, longitude, presensi_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [eventRows[0].id, nama, asal_instansi, jabatan, signature, catatan?.trim() || null, latitude ?? null, longitude ?? null]
    );
